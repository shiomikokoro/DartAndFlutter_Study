...
5. Stack 叠层布局
   ...
   1. 允许将多个子组件按照Z轴叠加排列。几乎在所有叠加效果的页面中出现，比如对话框、弹窗、操作菜单的浮层交互以及悬浮按钮等。

   2. Stack不能设置宽高，可以用具有宽高容器父组件包裹，对Stack进行设置。

   3. 叠层顺序是根据子组件顺序确定的，为逆相关。

   4. 常见属性

      1. Positioned - 直接子组件，对子组件进行精确定位控制
      2. alignment - 控制非Positioned的对齐方式
      3. clipBehavior - 控制超出Stack显示范围子组件的裁剪方式
      4. children - 子组件列表
